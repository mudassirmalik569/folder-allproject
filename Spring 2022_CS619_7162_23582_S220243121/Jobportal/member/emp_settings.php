...
$sql = "UPDATE users SET name = \"$username\", email = \"$email\", password = \"$password\", contact = \"$phone\", image = \"$user_image_1\", dob = \"$dob\", gender = \"$gender\", location = \"$location\", functional_area = \"$functional_area\" WHERE id = \"$user_id\"  ";
if($db->error){
  $error = $db->error;
  echo $error;
}
