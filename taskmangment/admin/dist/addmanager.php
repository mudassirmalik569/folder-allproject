...
{
   $validate = "select * from manager where fullname='$name'";
   $rsvalidate = mysqli_query($link,$validate);
   $value = mysqli_num_rows($rsvalidate);

   if($value==0){
   $query = "INSERT INTO manager (fullname,contact,city,email,password) 
   ...
   echo "<script>window.open('managers.php','_self')</script>";
   }
}
else{
